<template>
  <div class="user-list-container">
      ...
      >
        ...
        <el-table-column label="姓名" min-width="120" show-overflow-tooltip>
          <template #default="{ row }">
            <span>{{ row.real_name ?? row.realName ?? '' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="role" label="角色" width="100" align="center">
          <template #default="scope">
            <el-tag :type="getRoleTag(scope.row.role)" size="small" class="role-tag">
        ...
        <el-table-column label="创建时间" min-width="180" align="center" show-overflow-tooltip>
          <template #default="{ row }">
            <span>{{ row.created_at ?? row.createdAt ?? '' }}</span>
          </template>
        </el-table-column>
    ...
    <!-- 重置密码对话框 -->
    <el-dialog title="重置密码" v-model="resetDialog.visible" width="400px">
    ...
    <el-dialog title="分配角色" v-model="roleDialog.visible" width="500px">
      <el-form label-width="100px">
        <el-form-item label="用户">
          <span>{{ roleDialog.user?.real_name || roleDialog.user?.realName || roleDialog.user?.username }}</span>
        </el-form-item>
        <el-form-item label="角色">
          <el-select
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="roleDialog.loading" @click="handleRoleSave">
          保存
        </el-button>
      </template>
    </el-dialog>
...
import { Plus, Search, Download, Refresh, Edit, Delete } from '@element-plus/icons-vue'
import { getUsers, deleteUser, resetPassword } from '../api/user'
import { getRoleList, getUserRoleIds, saveUserRoles } from '../api/rbac'
...
function normalizeUser(u) {
  return {
    // 主键兼容：user_id / userId / id
    user_id: u.user_id ?? u.userId ?? u.id,

    // 字段原样
    username: u.username ?? '',

    // 姓名兼容：real_name / realName
    ...
    // 创建时间兼容：created_at / createdAt / createTime
    created_at: u.created_at ?? u.createdAt ?? u.createTime ?? '',
...
async function loadUsers() {
  ...
  try {
    const params = {
      ...
      keyword: (filterForm.username || filterForm.realName) || undefined
    }
    ...
    // 修复总数：优先取 data.total，其次 body.total
    pagination.total = Number(data?.total ?? body?.total ?? data?.count ?? body?.count ?? records.length)

    // 同步分页信息（有就用，没有就保留）
    pagination.current = data?.current ?? data?.page ?? pagination.current
    pagination.size = data?.size ?? data?.pageSize ?? pagination.size
  } catch (e) {
    console.error(e)
    users.value = []
    pagination.total = 0
    ElMessage.error('加载用户列表失败（请检查接口是否通、返回结构是否匹配）')
  } finally {
    loading.value = false
  }
}
...
async function loadRoles() {
  try {
    const res = await getRoleList({ pageSize: 100, pageNum: 1 })
    if (res.code === 200) {
      // 假设返回结构为 { code: 200, data: { records: [...] } }
...
async function handleAssignRoles(user) {
  roleDialog.user = user
  roleDialog.loading = true
  try {
    const res = await getUserRoleIds(user.user_id)
    if (res.code === 200) {
...
async function handleDelete(user) {
  try {
    await ElMessageBox.confirm(
      `确定删除用户 "${user.real_name || user.username}" 吗？此操作不可恢复。`,
      '删除确认',
      {
...
async function handleRoleSave() {
  if (!roleDialog.user?.user_id) {
    ElMessage.error('用户信息丢失')
    return
  }
  roleDialog.loading = true
  try {
    const res = await saveUserRoles(roleDialog.user.user_id, selectedRoles.value)
    if (res.code === 200) {
...
async function handleResetSubmit() {
  ...
  if (!resetDialog.user?.user_id) {
    ElMessage.error('未选择要重置的用户')
    return
  }

  resetDialog.loading = true
  try {
    await resetPassword(resetDialog.user.user_id, resetForm.newPassword)
    ElMessage.success('密码重置成功')
    resetDialog.visible = false
  } catch (error) {
    ElMessage.error('密码重置失败（请确认后端接口已实现）')
  } finally {
